...
class CustomResultSettingController extends Controller
{
    ...
    public function index()
    {
        ...
        try {
            ...
            $custom_settings = CustomResultSetting::where('academic_id', getAcademicId())
                    ->where('school_id', Auth::user()->school_id)
                    ->get();

            $system_setting= SmGeneralSettings::where('school_id',Auth::user()->school_id)->first();

            $check_exist= CustomResultSetting::where('academic_year','=',$system_setting->session_id)
            ->first();

            $edit_data= CustomResultSetting::where('academic_id', getAcademicId())
                        ->where('school_id', Auth::user()->school_id)
                        ->count();
            ...
            // }
            // return view('backEnd.systemSettings.custom_result_settings', compact('exams','custom_settings'));
    ...
    {
        try{
        //    $old =  CustomResultSetting::where('academic_id', getAcademicId())
        //                     ->where('school_id', Auth::user()->school_id)
        //                     ->get();

        //                     if($old){
        //                         CustomResultSetting::where('academic_id', getAcademicId())
        //                             ->where('school_id', Auth::user()->school_id)
        //                             ->delete();
        //                     }
    ...
    public function edit($id)
    {
        ...
        try {
            ...
            $exams = SmExamType::where('active_status', 1)->where('school_id', Auth::user()->school_id)->where('academic_id',getAcademicId())->get();
    ...
    {
        try {
            $system_setting = SmGeneralSettings::where('school_id', Auth::user()->school_id)->where('academic_id',getAcademicId())->first();
            $system_setting = $system_setting->session_id;
