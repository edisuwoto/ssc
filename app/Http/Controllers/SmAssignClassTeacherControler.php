...
class SmAssignClassTeacherControler extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            $assigned_class_teacher = SmAssignClassTeacher::where('active_status', 1)->where('class_id', $request->class)->where('section_id', $request->section)->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->first();
