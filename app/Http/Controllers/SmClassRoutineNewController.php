...
class SmClassRoutineNewController extends Controller
{
    ...
    public function addNewClassRoutine($class_time_id, $day, $class_id, $section_id)
    {
        ...
        try {
            ...
            $assinged_rooms = SmClassRoutineUpdate::select('room_id')->where('class_period_id', $class_time_id)
            ->where('day', $day)
    ...
    public function getClassTeacherAjax(Request $request)
    {
        ...
        try {
          $already_assigned = SmClassRoutineUpdate::select('teacher_id')->where('class_period_id', $request->class_time_id)
                            ->where('day', $request->day)
                            ->get();

            $subject_teacher = SmAssignSubject::select('teacher_id')
...
//            Toastr::error('Operation Failed', 'Failed');
    ...
    public function getOtherDaysAjax(Request $request){
        ...
        try {
            $subject_teacher = SmAssignSubject::select('teacher_id')->where('class_id', $request->class_id)
            ->where('section_id', $request->section_id)->where('subject_id', $request->subject)
            ->where('school_id',Auth::user()->school_id)->where('academic_id', getAcademicId())
            ->first();         
           ...
           $assgin_day_ids=$assgin_days->select('day')->get();
            ...
            $days=$days->where('is_weekend',0)->orderBy('order', 'ASC')->where('active_status', 1)->where('school_id',Auth::user()->school_id)->where('academic_id', getAcademicId())->get();
