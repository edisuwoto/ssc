...
class SmStudentAdmissionController extends Controller
{
    ...
    public function studentCurrentSearch(Request $request)
    {
        ...
        try {
            ...
            // $marks = SmMarkStore::where('class_id',$request->current_class)
            //                         ->where('section_id',$request->section)
            //                         ->where('exam_term_id', $request->exam)
            //                         ->get();

            //         $grades = SmMarksGrade::all();
            //         $fail_mark= collect($grades)->min("percent_upto");   
            //         $students = [];
            //         if ($request->result == 'P') {
            //             foreach($marks as $mark){
            //                 if($mark->total_marks > $fail_mark ){
       ...
       $input = $request->all();
        ...
        try {
            ...
            if ($request->promote_class == "" || $request->promote_session == "") {
                ...
                try {
                    $std_info = [];
                    foreach ($request->student_checked as $student_id) {
