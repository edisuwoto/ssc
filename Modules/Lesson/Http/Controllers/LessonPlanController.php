...
class LessonPlanController extends Controller
{
    ...
    public function lessonPlannerLesson($class_time_id, $day, $class_id, $section_id, $subject_id, $room_id, $assigned_id, $teacher_id,$lesson_date)
    {
        ...
        try {
            ...
            return view('lesson::lessonPlan.add_new_lesson_planner_form', compact('teacher_id','lesson_date','rooms','lessons', 'subjects', 'day', 'class_time_id', 'class_id', 'section_id', 'assinged_subject', 'assinged_room', 'subject_id', 'room_id', 'assigned_id', 'teacher_detail'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
    ...
    public function addNewLessonPlan(Request $request)
    {
        ...
        $lessonPlanner->room_id=1;
        $lessonPlanner->teacher_id=$request->teacher_id;
        $lessonPlanner->class_period_id=$request->class_time_id;
    ...
    public function ViewlessonPlannerLesson($lessonPlan_id,$class_time_id, $day, $class_id, $section_id, $subject_id, $room_id, $assigned_id, $teacher_id,$lesson_date){
        try {
    ...
    public function EditlessonPlannerLesson($lessonPlan_id,$class_time_id, $day, $class_id, $section_id, $subject_id, $room_id, $assigned_id, $teacher_id,$lesson_date){
        try {
            $lesson_date=$lesson_date;
    ...
    public function changeWeek(Request $request,$teacher_id,$next_date){
			...
			$user = Auth::user();
			$class_times = SmClassTime::where('type','class')->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->orderBy('period', 'ASC')->get();
            if($user->role_id == 4){
    ...
    public function topicOverview(){
        try {
            $lessons=SmLesson::where('academic_id', getAcademicId())->groupby('lesson_title')->where('school_id', Auth::user()->school_id)->get();
      
            $topics_detail=SmLessonTopicDetail::where('academic_id', getAcademicId())
            ->where('school_id', Auth::user()->school_id)
            ->get();
    ...
    public function topicOverviewSearch(Request $request)
    {
        try {
            ...
            $lessons=SmLesson::where('academic_id', getAcademicId())->groupby('lesson_title')->where('school_id', Auth::user()->school_id)->get();
            $topics_detail=SmLessonTopicDetail::where('academic_id', getAcademicId())->where('lesson_id',$request->lesson)->where('school_id', Auth::user()->school_id)->get();
    ...
    public function searchLessonPlan(Request $request){
       ...
       try{ 
        ...
        $classes = SmClass::where('active_status', 1)
         ...
         $sectionIds = SmClassSection::where('class_id', '=', $class_id)
                 ->where('academic_id', getAcademicId())
            ...
            $subjectIds = SmAssignSubject::where('class_id', $class_id)->where('section_id', $section_id)->where('school_id',Auth::user()->school_id)->get(); 
            $subjects = [];
    ...
    public function completeStatus(Request $request){
        ...
        $lessonPlan_id=$request->lessonPlan_id;
    ...
    public function teacherLessonPlanOverview(){
        if(Auth::user()->role_id==4){
            try {
                ...
                $user = Auth::user();
                $class_times = SmClassTime::where('type','class')->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->orderBy('id', 'ASC')->get();
    ...
    public  function searchlessonPlanReport(Request $request){
        ...
        $subject_id=$request->subject;
            ...
            $subjectIds = SmAssignSubject::where('teacher_id', $teacher_id)->where('school_id',Auth::user()->school_id)->get(); 
            $subjects = [];
            foreach ($subjectIds as $subjectId) {
                $subjects[] = SmSubject::find($subjectId->subject_id);
                 }
    ...
    public function loadDefault()
    {
        ...
        foreach ($data['period'] as $date){
         ...
         $data['sm_weekends'] = SmWeekend::where('school_id', Auth::user()->school_id)->orderBy('order', 'ASC')->where('active_status', 1)->get();
